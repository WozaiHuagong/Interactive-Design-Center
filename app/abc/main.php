...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
		if (is_mobile())
		{
			HTTP::redirect('/m/abc/' . $_GET['id']);
		}

		if ($_GET['column'] == 'log' AND !$this->user_id)
		{
			HTTP::redirect('/abc/' . $_GET['id']);
		}
		...
		$question_info['redirect'] = $this->model('abc')->get_redirect($question_info['question_id']);

		if ($question_info['redirect']['target_id'])
		{
			$target_question = $this->model('abc')->get_question_info_by_id($question_info['redirect']['target_id']);
		}

		if (is_digits($_GET['rf']) and $_GET['rf'])
		{
			if ($from_question = $this->model('abc')->get_question_info_by_id($_GET['rf']))
			{
		...
		if ($question_info['redirect'] and ! $_GET['rf'])
		{
			if ($target_question)
			{
				HTTP::redirect('/abc/' . $question_info['redirect']['target_id'] . '?rf=' . $question_info['question_id']);
			}
			else
			{
		...
		else if ($question_info['redirect'])
		{
			if ($target_question)
			{
				$message = AWS_APP::lang()->_t('此问题将跳转至') . ' <a href="' . get_js_url('/abc/' . $question_info['redirect']['target_id'] . '?rf=' . $question_info['question_id']) . '">' . $target_question['question_content'] . '</a>';
				...
				{
					$message .= '&nbsp; (<a href="javascript:;" onclick="AWS.ajax_request(G_BASE_URL + \'/abc/ajax/redirect/\', \'item_id=' . $question_info['question_id'] . '\');">' . AWS_APP::lang()->_t('撤消重定向') . '</a>)';
				}
		...
		{
			$question_info['attachs'] = $this->model('publish')->get_attach('ab_question', $question_info['question_id'], 'min');
		...
		if ($_GET['column'] != 'log')
		{
			$this->model('abc')->calc_popular_value($question_info['question_id']);
			$this->model('abc')->update_views($question_info['question_id']);
			...
			{
				$answer_count = $this->model('abc')->get_answer_count_by_question_id($question_info['question_id'], $answer_count_where);
			}
			else
			{
			...
			{
				$answer_list = $this->model('abc')->get_answer_list_by_question_id($question_info['question_id'], 1, 'answer_id = ' . intval($_GET['answer_id']));
			}
			else if (! $this->user_id AND !$this->user_info['permission']['answer_show'])
			{
				if ($question_info['best_answer'])
				{
					$answer_list = $this->model('abc')->get_answer_list_by_question_id($question_info['question_id'], 1, 'answer_id = ' . intval($question_info['best_answer']));
				}
				else
				{
					$answer_list = $this->model('abc')->get_answer_list_by_question_id($question_info['question_id'], 1, null, 'agree_count DESC');
				}
			}
			else
			{
				...
				$answer_list = $this->model('abc')->get_answer_list_by_question_id($question_info['question_id'], calc_page_limit($_GET['page'], 100), $answer_list_where, $answer_order_by);
			}
			...
			{
				$answer_list = array_merge($this->model('abc')->get_answer_list_by_question_id($question_info['question_id'], 1, 'answer_id = ' . $question_info['best_answer']), $answer_list);
			}

			if ($answer_ids)
			{
				$answer_attachs = $this->model('publish')->get_attachs('ab_answer', $has_attach_answer_ids, 'min');
			}
			...
			foreach ($answer_list as $answer)
			{
				...
				$answer['answer_content'] = $this->model('abc')->parse_at_user(FORMAT::parse_attachs(nl2br(FORMAT::parse_bbcode($answer['answer_content']))));
			...
			{
				if ($this->model('abc')->has_answer_by_uid($question_info['question_id'], $this->user_id))
				{
        ...
        //其他整理
        $other_sortout = $this->model('abc')->fetch_all('sortout', 'question_id = ' . $question_info['question_id'], null, 5);
		...
		$this->crumb($question_info['question_content'], '/abc/' . $question_info['question_id']);

		if ($_GET['column'] == 'log')
		{
			$this->crumb(AWS_APP::lang()->_t('日志'), '/abc/id-' . $question_info['question_id'] . '__column-log');
		}
		else
		{
			...
			if ($this->user_id)
			{
				TPL::assign('pagination', AWS_APP::pagination()->initialize(array(
					'base_url' => get_js_url('/abc/id-' . $question_info['question_id'] . '__sort_key-' . $_GET['sort_key'] . '__sort-' . $_GET['sort'] . '__uid-' . $_GET['uid']),
		...
		switch ($question_info['category_id']) {
			case 1:
				# code...
				TPL::output('abc/A');
				break;
	...
	// 	if (TPL::is_output('block/sidebar_comment.tpl.htm', 'question/square'))
	// 	{
	// 		TPL::assign('feature_list', $this->model('module')->feature_list());
	// 	}
	...
	// 			break;
	// 		case 'shang':
	// 			$qsList = $this->model('question')->get_question_shang(5);
	// 			$question_list=[];
	// 			foreach ($qsList as $key => $value) {
 ...
 //        //TPL::assign('hot_sortout', $this->model('sortout')->get_sortouts_list(null, 1, 5, 'comments DESC'));
	// 	TPL::assign('question_shang', $this->model('question')->get_question_shang(5));
