...
class ajax extends AWS_CONTROLLER
{
	...
	{
		$answer_info = $this->model('abc')->get_answer_by_id($_GET['id']);
	...
	public function save_answer_comment_action()
	{
		...
		$answer_info = $this->model('abc')->get_answer_by_id($_GET['answer_id']);
		$question_info = $this->model('abc')->get_question_info_by_id($answer_info['question_id']);
		...
		$this->model('abc')->insert_answer_comment($_GET['answer_id'], $this->user_id, $_POST['message']);
	...
	{
		$comments = $this->model('abc')->get_answer_comments($_GET['answer_id']);
		...
		TPL::assign('question', $this->model('abc')->get_question_info_by_id($answer_info['question_id']));
	...
	public function save_question_comment_action()
	{
		...
		$question_info = $this->model('abc')->get_question_info_by_id($_GET['question_id']);
		...
		$this->model('abc')->insert_question_comment($_GET['question_id'], $this->user_id, $_POST['message']);
	...
	{
		$comments = $this->model('abc')->get_question_comments($_GET['question_id']);
	...
	public function save_answer_action()
	{
		...
		// 判断是否已回复过问题
		if ((get_setting('answer_unique') == 'Y') AND $this->model('abc')->has_answer_by_uid($question_info['question_id'], $this->user_id))
		{
		...
		$this->model('draft')->delete_draft($question_info['question_id'], 'ab_answer', $this->user_id);

		//H::ajax_json_output(AWS_APP::RSM(null, '-1', json_encode($_POST)));

		if ($this->publish_approval_valid($answer_content))
		{
			$this->model('publish')->publish_approval('ab_answer', array(
		...
		{
			$answer_id = $this->model('publish')->publish_ab_answer($question_info['question_id'], $answer_content, $this->user_id, $_POST['anonymous'], $_POST['attach_access_key'], $_POST['auto_focus']);

			// if ($_POST['_is_mobile'])
			// {
			// 	//$url = get_js_url('/m/question/id-' . $question_info['question_id'] . '__item_id-' . $answer_id . '__rf-false');

			// 	$this->model('abc')->set_answer_publish_source($answer_id, 'mobile');
			// }
			// else
			// {
			...
			$answer_info = $this->model('abc')->get_answer_by_id($answer_id);

			if ($answer_info['has_attach'])
			{
				$answer_info['attachs'] = $this->model('publish')->get_attach('ab_answer', $answer_id, 'min');

				$answer_info['insert_attach_ids'] = FORMAT::parse_attachs($answer_info['answer_content'], true);
			}

			$answer_info['user_info'] = $this->user_info;
			$answer_info['answer_content'] = $this->model('abc')->parse_at_user(FORMAT::parse_attachs(nl2br(FORMAT::parse_bbcode($answer_info['answer_content']))));
			...
			{
				H::ajax_json_output(AWS_APP::RSM(array(
					'ajax_html' => TPL::output('abc/ajax/answer', false)
				), 1, null));
			}
		}
	}
	...
	{
		if (! $answer_info = $this->model('abc')->get_answer_by_id($_GET['answer_id']))
		{
		...
		if ($_POST['do_delete'])
		{
			...
			$this->model('abc')->remove_answer_by_id($_GET['answer_id']);

			$this->model('abc')->save_last_answer($answer_info['question_id']);
		...
		$this->model('abc')->update_answer($_GET['answer_id'], $answer_info['question_id'], $answer_content, $_POST['attach_access_key']);
	...
	public function log_action()
	{
		...
		//处理日志记录
		$log_list = $this->model('abc')->analysis_log($log_list, $question_info['published_uid'], $question_info['anonymous']);
	...
	public function save_report_action()
	{
        ...
        if($_POST['type'] == 'info'){
            $this->model('ad')->update('ad', [
                'has_virus'   => $this->model('ad')->count('report', "type = '" . $_POST['type'] . "' AND target_id = " . intval($_POST['target_id']) . " AND reason like '%有病毒%'"),
                'has_ad'      => $this->model('ad')->count('report', "type = '" . $_POST['type'] . "' AND target_id = " . intval($_POST['target_id']) . " AND reason like '%有恶意广告%'"),
                'has_illegal' => $this->model('ad')->count('report', "type = '" . $_POST['type'] . "' AND target_id = " . intval($_POST['target_id']) . " AND reason like '%非法用途%'"),
	...
	public function list_action()
	{
		...
		//$question_list = $this->model('question')->get_question_list(intval($_GET['page']) * $per_page . ', ' . $per_page);
		if ($_GET['category'])
		{
		...
		switch ($_GET['sort_type']) {
			case 'hot':
				...
				break;
			case 'shang':
				$qsList = $this->model('question')->get_question_shang(5,($_GET['page']-1)*5);
				$question_list=[];
				foreach ($qsList as $key => $val) {
					$question_list[]=$val['question_info'];
				}
				break;
			default:
		...
		if ($question_list)
		{
			foreach ($question_list AS $key => $val)
			{
				if ($val['answer_count'])
				{
					...
					$question_list[$key]['answer_shang_list']=$this->model('question')->get_shang_answer_list_by_article_ids($val['question_id']);
					$question_list[$key]['shang_info']=$this->model('question')->get_question_shang(5,null,$val['question_id']);
					$question_list[$key]['zanzhu_count']=$this->model('question')->get_shang_person_num_by_question_id($val['question_id']);
				}
			}
		}
	...
	public function zanzhu_action(){
        if (!$question_info = $this->model('question')->get_question_info_by_id($_POST['question_id']))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, '问题不存在'));
        }

        if(!$this->model('account')->check_pay_password($_POST['password'], $this->user_id)){
        	H::ajax_json_output(AWS_APP::RSM(null, -1, '交易密码错误'));
        }
        
        $question_bounty = $_POST['zanzhu']*100;

        $shang = $this->model('question')->fetch_row('question_shang', 'type = \'question\' AND question_id = ' . $question_info['question_id'] . ' AND uid = ' . $this->user_id);

        if($shang['status'] != 0){
            H::ajax_json_output(AWS_APP::RSM(null, -1, '赞助金已分配,无法再次赞助'));
        }
        //没变化
        if($question_bounty == $shang['cash']){
            H::ajax_json_output(AWS_APP::RSM(null, -1, '赞助金额没有变化'));
        }

        $cash = $question_bounty - $shang['cash'];
        if($this->user_info['cash'] - $cash < 0){
            H::ajax_json_output(AWS_APP::RSM(null, -1, '账户余额不足,无法完成赞助'));
        }

        //打赏
        $this->model('shang')->shang_question($question_info['question_id'], $this->user_id, $question_bounty);
    ...
    public function shang_action(){
        ...
        if($_POST['type'] == 'answer'){
            //纯粹打赏给回答者。
            $shang_cash = $_POST['shang_cash'];
            $user_ids = $_POST['user_ids'];

            $total_cash = 0;
            foreach ($shang_cash as $key => $val){
                $total_cash += $val;
            }
            if(!$total_cash OR ($total_cash*100) > $this->user_info['cash']){
                H::ajax_json_output(AWS_APP::RSM(null, -1, '赏金余额不足,无法完成操作'));
            }
            ...
            $this->model('shang')->shang_user($user_ids, $shang_cash, $this->user_id, 0, 'answer', $question_info['question_id']);
            ...
            if($shang['status'] != 0){
                H::ajax_json_output(AWS_APP::RSM(null, -1, '赞助金已分配,无法再次分配'));
            }
            ...
            $user_shang_cash = intval($total_cash*100);

            if($user_shang_cash != $shang['cash']){

                H::ajax_json_output(AWS_APP::RSM(null, -1, '分配的赏金和您打赏的金额不一致,无法完成操作'));
            }

            foreach ($shang_cash as $key => $val){
                //打赏
                //shang_user($uid, $cash, $from_uid, $from_shang_id, $item_type, $item_id)
                $this->model('shang')->shang_user($user_ids[$key], $val*100, $this->user_id, $shang['id'], 'answer', $key);
                $this->model('shang')->update('answer', [
                    'shang_cash' => $this->model('shang')->sum('shang', 'cash', "item_type = 'answer' AND item_id = " . intval($key)),
                ], 'answer_id = ' . intval($key));
            }

            $this->model('shang')->update('question_shang', [
                'status' => 1,
            ], 'id = ' . $shang['id']);
        }
    ...
    public function shang_answer_action(){
        if (!$answer_info = $this->model('answer')->get_answer_by_id($_POST['answer_id']))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, '答案不存在'));
        }
        ...
        if(!$shang_cash OR $shang_cash*100 > $this->user_info['cash']){
            H::ajax_json_output(AWS_APP::RSM(null, -1, '赏金余额不足,无法完成操作'));
        }

        //打赏
        //shang_user($uid, $cash, $from_uid, $from_shang_id, $item_type, $item_id)
        $this->model('shang')->shang_user($answer_info['uid'], $shang_cash*100, $this->user_id, 0, 'answer', $answer_info['answer_id']);

        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }

    public function shang_sortout_action(){
        if (!$sortout_info = $this->model('shang')->fetch_row('sortout', 'id = ' . intval($_POST['sortout_id'])))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, '整理不存在'));
        }
        ...
        //打赏
        //shang_user($uid, $cash, $from_uid, $from_shang_id, $item_type, $item_id)
        $this->model('shang')->shang_user($sortout_info['uid'], $shang_cash*100, $this->user_id, 0, 'sortout', $sortout_info['id']);

        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }

    public function shang_article_action(){
        if (!$article_info = $this->model('shang')->fetch_row('article', 'id = ' . intval($_POST['article_id'])))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, '文章不存在'));
        }
        ...
        //打赏
        //shang_user($uid, $cash, $from_uid, $from_shang_id, $item_type, $item_id)
        $this->model('shang')->shang_user($article_info['uid'], $shang_cash*100, $this->user_id, 0, 'article', $article_info['id']);

        H::ajax_json_output(AWS_APP::RSM(null, 1, null));
    }

    public function shang_comment_action(){
        if($_POST['type'] == 'article_comment'){
            if (!$comment_info = $this->model('shang')->fetch_row('article_comments', 'id = ' . intval($_POST['comment_id'])))
            {
                H::ajax_json_output(AWS_APP::RSM(null, -1, '文章评论不存在'));
            }
        }else{
            $_POST['type'] = 'sortout_comment';
            if (!$comment_info = $this->model('shang')->fetch_row('sortout_comments', 'id = ' . intval($_POST['comment_id'])))
            {
                H::ajax_json_output(AWS_APP::RSM(null, -1, '整理文章评论不存在'));
            }
        }
        ...
        //打赏
        //shang_user($uid, $cash, $from_uid, $from_shang_id, $item_type, $item_id)
        $this->model('shang')->shang_user($comment_info['uid'], $shang_cash*100, $this->user_id, 0, $_POST['type'], $comment_info['id']);
