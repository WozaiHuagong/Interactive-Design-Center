...
class ajax extends AWS_CONTROLLER
{
	...
	public function user_actions_action()
	{
		...
		else if ($_GET['actions'] == '301')
		{
			$article_list = $this->model('article')->get_articles_list_by_uid('',intval($_GET['page']+1),get_setting('contents_per_page'), 'add_time DESC','',$_GET['uid']);
			TPL::assign('article_list', $article_list);

			TPL::output($template_dir . '/ajax/user_actions_article_301');
		}
		else if ($_GET['actions'] == '302')
		{
			$sortout_list = $this->model('sortout')->get_sortouts_list_by_uid('',intval($_GET['page']+1),get_setting('contents_per_page'), 'add_time DESC','',$_GET['uid']);
			TPL::assign('article_list', $sortout_list);

			TPL::output($template_dir . '/ajax/user_actions_article_302');
		}
		else if ($_GET['actions'] == '303')
		{
			$prize_list = $this->model('prize')->get_prizes_list_by_uid('',intval($_GET['page']+1),get_setting('contents_per_page'), 'add_time DESC','',$_GET['uid']);
			TPL::assign('article_list', $prize_list);

			TPL::output($template_dir . '/ajax/user_actions_article_303');
		}
		else
		{
