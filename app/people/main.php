...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
		...
		$data = $this->model('actions')->get_user_actions($user['uid'], (intval($_GET['page']) * $this->per_page) . ", {$this->per_page}", '101', $this->user_id);
		TPL::assign('counta', count($data));

		$data = $this->model('actions')->get_user_actions($user['uid'], (intval($_GET['page']) * $this->per_page) . ", {$this->per_page}", '201', $this->user_id);
		TPL::assign('countq', count($data));

		$article_list = $this->model('article')->get_articles_list_by_uid('',intval($_GET['page']+1),get_setting('contents_per_page'), 'add_time DESC','',$user['uid']);
		TPL::assign('countarticle', count($article_list));

		$sortout_list = $this->model('sortout')->get_sortouts_list_by_uid('',intval($_GET['page']+1),get_setting('contents_per_page'), 'add_time DESC','',$user['uid']);
		TPL::assign('countsortout', count($sortout_list));

		$prize_list = $this->model('prize')->get_prizes_list_by_uid('',intval($_GET['page']+1),get_setting('contents_per_page'), 'add_time DESC','',$user['uid']);
		TPL::assign('countprize', count($prize_list));
