...
class ajax extends AWS_ADMIN_CONTROLLER
{
    ...
    public function save_settings_action()
    {
        ...
        //保存几个图片
        if ($_FILES['ask_thumb']['name']) {
            AWS_APP::upload()->initialize(array(
                'allowed_types' => 'jpg,jpeg,png,gif',
                'upload_path' => get_setting('upload_dir') . '/images',
                'is_image' => TRUE
            ))->do_upload('ask_thumb');
            ...
            $save_data['ask_thumb'] = '/images/' . basename($upload_data['full_path']);
        }
        if ($_FILES['sortout_thumb']['name']) {
            AWS_APP::upload()->initialize(array(
                'allowed_types' => 'jpg,jpeg,png,gif',
                'upload_path' => get_setting('upload_dir') . '/images',
                'is_image' => TRUE
            ))->do_upload('sortout_thumb');
            ...
            $save_data['sortout_thumb'] = '/images/' . basename($upload_data['full_path']);
        }
        if ($_FILES['prize_thumb']['name']) {
            AWS_APP::upload()->initialize(array(
                'allowed_types' => 'jpg,jpeg,png,gif',
                'upload_path' => get_setting('upload_dir') . '/images',
                'is_image' => TRUE
            ))->do_upload('prize_thumb');
            ...
            $save_data['prize_thumb'] = '/images/' . basename($upload_data['full_path']);
        }
        if ($_FILES['article_thumb']['name']) {
            ...
            $save_data['article_thumb'] = '/images/' . basename($upload_data['full_path']);
        }
    ...
    public function article_manage_action()
    {
        ...
        switch ($_POST['action'])
        {
            ...
            case 'bind':
                ...
                {
                    $this->model('elite')->bind_elite_articles_to_users($article_id,$_POST['elite_ids'][$index]);
                }
                H::ajax_json_output(AWS_APP::RSM(null, 1, null));
            break;
        }
    ...
    public function sortout_manage_action()
    {
        if (!$_POST['sortout_ids'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择整理文章进行操作')));
        }

        switch ($_POST['action'])
        {
            case 'del':
                foreach ($_POST['sortout_ids'] AS $sortout_id)
                {
                    $this->model('sortout')->remove_sortout($sortout_id);
                }
    ...
    public function save_carousel_action()
    {
        if (!$this->user_info['permission']['is_administortar'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('你没有访问权限, 请重新登录')));
        ...
        if ($_FILES['file'] == ''){
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请选择轮播图')));
        }

        if (trim($_POST['content']) == '')
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请输入轮播图小标题')));
        }
        ...
        $category = $this->model('carousel')->put_carousel($this->user_id,$show_index,$url,$_POST['title'],$_POST['content']);
    ...
    public function update_carousel_action()
    {
        if (!$this->user_info['permission']['is_administortar'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('你没有访问权限, 请重新登录')));
    ...
    public function remove_carousel_action()
    {
        if (!$this->user_info['permission']['is_administortar'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('你没有访问权限, 请重新登录')));
        ...
        $this->model('carousel')->del_carousel($_POST['carousel_id']);
    ...
    public function save_topic_action()
    {
        ...
        $this->model('topic')->set_type($topic_id, $_POST['type']);
    ...
    public function save_approval_item_action()
    {
        ...
        if (!$_POST['title'] AND in_array($_POST['type'], array('question', 'article', 'sortout', 'received_email')))
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('请输入标题')));
        }

        if (!$_POST['content'] AND in_array($_POST['type'], array('answer', 'article_comment', 'sortout_comment')))
        {
        ...
        if ($approval_item['type'] != 'sortout_comment' AND $_POST['remove_attachs'])
        {
            foreach ($_POST['remove_attachs'] AS $attach_id)
            {
                $this->model('publish')->remove_attach($attach_id, $approval_item['data']['attach_access_key']);
