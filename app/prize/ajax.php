...
class ajax extends AWS_CONTROLLER
{
	...
	public function save_comment_action()
	{
		...
		{
			$this->model('publish')->publish_approval('prize_comment', array(
		...
		{
			$comment_id = $this->model('publish')->publish_prize_comment($_POST['article_id'], $message, $this->user_id, $_POST['at_uid']);

			//$url = get_js_url('/article/' . intval($_POST['article_id']) . '?item_id=' . $comment_id);

			$comment_info = $this->model('prize')->get_comment_by_id($comment_id);
			...
			{
				H::ajax_json_output(AWS_APP::RSM(array(
					'ajax_html' => TPL::output('m/ajax/prize_answer', false)
				), 1, null));
			}
			else
			{
				H::ajax_json_output(AWS_APP::RSM(array(
					'ajax_html' => TPL::output('prize/ajax/comment', false)
				), 1, null));
			}
		}
	}
	...
	public function lock_action()
	{
		...
		$this->model('prize')->lock_prize($_POST['article_id'], !$article_info['lock']);
	...
	public function remove_comment_action()
	{
		...
		if ($comment_info = $this->model('prize')->get_comment_by_id($_POST['comment_id']))
		{
			$this->model('prize')->remove_comment($comment_info['id']);
		}

		H::ajax_json_output(AWS_APP::RSM(array(
			'url' => get_js_url('/prize/' . $comment_info['article_id'])
		), 1, null));
	}
	...
	public function article_vote_action()
	{
		...
		{
			case 'prize':
				$item_info = $this->model('prize')->get_article_info_by_id($_POST['item_id']);
			break;

			case 'comment':
				$item_info = $this->model('prize')->get_comment_by_id($_POST['item_id']);
			break;
		...
		$this->model('prize')->prize_vote($_POST['type'], $_POST['item_id'], $_POST['rating'], $this->user_id, $reputation_factor, $item_info['uid']);
	...
	public function set_recommend_action()
	{
		...
		{
			case 'set':
				$this->model('prize')->set_recommend($_POST['article_id']);
			break;

			case 'unset':
				$this->model('prize')->unset_recommend($_POST['article_id']);
			break;
		}
