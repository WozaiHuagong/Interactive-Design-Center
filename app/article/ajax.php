...
class ajax extends AWS_CONTROLLER
{
	...
	public function list_action()
	{
		...
		// switch ($_GET['type']) {
		// 	case 'shang':
		// 		//$article_list = $this->model('article')->get_articles_list($category_info['id'], $_GET['page'], get_setting('contents_per_page'), 'add_time DESC',null,$_GET['type']);
		// 		$article_table=$this->model('article')->get_table('article');
		// 		$shang_table=$this->model('article')->get_table('shang');
		// 		$article_list = $this->model('article')->query_all("SELECT * FROM $article_table LEFT JOIN (SELECT id as sid,item_id as uid from $shang_table where item_type = 'article') s ON id=s.uid  where shang_cash > 0 ORDER BY sid DESC",$_GET['page']-1,($_GET['page']-1)*get_setting('contents_per_page'));
		// 		break;
		// 	case 'new':
		// 	default:
		...
		$article_table=$this->model('article')->get_table('article');
		$shang_table=$this->model('article')->get_table('shang');
		$article_list = $this->model('article')->query_all("SELECT *,CASE WHEN ISNULL(new_time) OR add_time > new_time THEN add_time ELSE new_time END as order_time FROM $article_table LEFT JOIN (SELECT max(add_time) as new_time,item_id from $shang_table WHERE item_type = 'article' GROUP BY item_id) s ON id=s.item_id GROUP BY id ORDER BY order_time DESC ",$_GET['page']-1,($_GET['page']-1)*get_setting('contents_per_page'));
		foreach ($article_list as $key => $article) {
			$article_list[$key]['comment_message']=$this->model('article')->get_shang_comments_list_by_article_id($article['id']);
			$article_list[$key]['shang_time']=$this->model('article')->get_shang_person_num_by_article_id($article['id']);
		}
