...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
		...
		TPL::assign('category_information', $this->model('topic')->get_topic_by_id($article_info['category_id']));
		...
		TPL::assign('keywords', $this->model('topic')->get_topic_by_title($article_info['title']));
		...
		//作者文章
		$article_list_byuser = $this->model('article')->get_articles_list_by_uid('',1,5, 'add_time DESC','',$article_info['user_info']['uid']);
		TPL::assign('article_list_byuser', $article_list_byuser);
		//相关文章
		$article_list = $this->model('article')->get_articles_list($article_info['category_id'],1, 5, 'add_time DESC');
	...
	public function index_square_action()
	{
        ...
        //精彩标签
        TPL::assign('hot_tag', $this->model('topic')->get_topic_list('type = \'article\''));

		//公告
		// $infos = get_setting('info_list');
		// $info_arg = explode(',',$infos);
		$A=$this->model('abc')->get_question_list(1,1,'article');//A
		$B=$this->model('abc')->get_question_list(1,2,'article');//B
