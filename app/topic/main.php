			...
			default:
				...
				if ($posts_list = $this->model('posts')->get_posts_list(null, 1, get_setting('contents_per_page'), 'new', $contents_related_topic_ids))
				{
					foreach ($posts_list AS $key => $val)
					{
						...
						if ($val['views']){//为article
							$result=$this->model('posts')->get_post_by_post_id($val['id']);
							$posts_list[$key]=array_merge($posts_list[$key],$result);
						}
					}
				}

				
				if($topic_info["type"]=="question"){
					TPL::assign('posts_list', $posts_list);
					TPL::assign('all_list_bit', TPL::output('question/ajax/list', false));
				}
				else{
					foreach ($posts_list as $key => $article) {
						$posts_list[$key]['comment_message']=$this->model('article')->get_shang_comments_list_by_article_id($article['post_id']);
						$posts_list[$key]['shang_time']=$this->model('article')->get_shang_person_num_by_article_id($article['post_id']);
					}
					TPL::assign('article_list', $posts_list);
					TPL::assign('all_list_bit', TPL::output('article/ajax/list', false));
				}
