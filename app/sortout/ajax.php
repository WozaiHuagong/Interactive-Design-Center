...
class ajax extends AWS_CONTROLLER
{
	...
	public function save_comment_action()
	{
		...
		{
			H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('指定文章不存在111')));
		}
		...
		{
			$this->model('publish')->publish_approval('sortout_comment', array(
		...
		{
			$comment_id = $this->model('publish')->publish_sortout_comment($_POST['article_id'], $message, $this->user_id, $_POST['at_uid']);

			//$url = get_js_url('/article/' . intval($_POST['article_id']) . '?item_id=' . $comment_id);

			$comment_info = $this->model('sortout')->get_comment_by_id($comment_id);
			...
			{
				H::ajax_json_output(AWS_APP::RSM(array(
					'ajax_html' => TPL::output('m/ajax/sortout_answer', false)
				), 1, null));
			}
			else
			{
				H::ajax_json_output(AWS_APP::RSM(array(
					'ajax_html' => TPL::output('sortout/ajax/comment', false)
				), 1, null));
			}
		}
	}
	...
	public function list_action()
	{
		...
		foreach ($article_list as $key => $val) {
			# code...
        	$shang_list=$this->model('people')->fetch_all('shang', 'cash > 0 AND item_type = \'answer\' and from_uid = ' . intval($article_list[$key]["uid"]) . ' AND from_question_shang = ' . intval($article_list[$key]["shang_id"]));
        	$sum_shang=$this->model('people')->sum('shang','cash','cash > 0 AND item_type = \'answer\' and from_uid = ' . intval($article_list[$key]["uid"]) . ' AND from_question_shang = ' . intval($article_list[$key]["shang_id"]));
        	$person_num=count($shang_list);
        	$output=[];
        	foreach ($shang_list as $value) {
        		$output[]=$value["cash"]/$sum_shang;
        	}
        	$article_list[$key]["shang_person_num"]=$person_num;
        	$article_list[$key]["shang_list"]=$output;
			$article_list[$key]['shang_time']=$this->model('sortout')->get_shang_time_with_sortout_id($val['id']);
		}
		TPL::assign('article_list', $article_list);

		TPL::output('sortout/ajax/list');
