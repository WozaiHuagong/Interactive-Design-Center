...
class ajax extends AWS_CONTROLLER
{
	...
	public function finance_action(){
        ...
        }else if($_GET['action'] == 'in'){
            $action = finance_class::ACTION_IN;
        }
        $list = $this->model('finance')->get_finance_list($this->user_id, $type, $action, $_GET['page'], get_setting('contents_per_page'));

        TPL::assign('list', $list);

        TPL::output('home/ajax/finance');
    }

    public function pay_password_action(){
        if (!$_POST['old_password'])
        {
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('请输入当前交易密码')));
        }
        ...
        {
            H::ajax_json_output(AWS_APP::RSM(null, -1, AWS_APP::lang()->_t('交易密码长度不符合规则')));
        }

        if ($this->model('account')->update_user_pay_password($_POST['password'], $this->user_id, $_POST['old_password']))
        {
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('交易密码修改成功, 请牢记新密码')));
        }
        else
        {
            H::ajax_json_output(AWS_APP::RSM(null, '-1', AWS_APP::lang()->_t('请输入正确的当前交易密码')));
        }
    }
    ...
    public function ad_click_action(){
        ...
        $where = 'ad_id = ' . $ad['id'];
        $list = $this->model('finance')->get_ad_click_list($_GET['page'], get_setting('contents_per_page'), $where);

        TPL::assign('list', $list);

        TPL::output('home/ajax/ad_click');
    }
}