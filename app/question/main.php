...
class main extends AWS_CONTROLLER
{
	...
	public function index_action()
	{
        ...
        //其他整理
        $other_sortout = $this->model('question')->fetch_all('sortout', 'question_id = ' . $question_info['question_id'], null, 5);
		...
		//赏金
        $shang = $this->model('shang')->fetch_row('question_shang', 'type = \'question\' AND question_id = ' . $question_info['question_id'] . ' AND uid = ' . $question_info['published_uid']);
        TPL::assign('shang', $shang);
        //其他加赏
        $other_shang = $this->model('shang')->fetch_all('question_shang', 'cash > 0 AND type = \'question\' AND question_id = ' . $question_info['question_id'] . ' AND uid != ' . $question_info['published_uid']);
        foreach ($other_shang as $key => $val){
            $other_shang[$key]['user_info'] = $this->model('account')->get_user_info_by_uid($val['uid'], true);
            $other_shang[$key]['user_follow_check'] = $this->model('follow')->user_follow_check($this->user_id, $val['uid']);
        }
        TPL::assign('other_shang', $other_shang);
        //当前用户打赏
        if($this->user_id){
            $user_shang = $this->model('shang')->fetch_row('question_shang', 'type = \'question\' AND status = 0 AND question_id = ' . $question_info['question_id'] . ' AND uid = ' . $this->user_id);
            TPL::assign('user_shang', $user_shang);
        }
	...
	{
		$this->crumb(AWS_APP::lang()->_t('问答'), '/question/');
		...
		$A=$this->model('abc')->get_question_list(1,1,'question');//A
		$B=$this->model('abc')->get_question_list(1,2,'question');//B
