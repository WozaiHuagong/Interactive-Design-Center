...
class article_class extends AWS_MODEL
{
	...
	public function update_article($article_id, $uid, $title, $message, $topics, $category_id, $create_topic, $award_price, $is_original, $is_award, $summary,$other_category,$show_index)
	{
	...
	public function get_articles_list($category_id, $page, $per_page, $order_by, $day = null,$type=null)
	{
		$where = array();
		switch ($type) {
			case 'shang':
	...
	public function get_articles_list_by_uid($category_id, $page, $per_page, $order_by, $day = null, $uid = null)
	{
		$where = array();
	...
	public function get_comments_list($order = 'votes DESC')
	{
		$comments = $this->fetch_page('article_comments', '', $order, 1,5);

		return $comments;
	}
	public function get_shang_comments_list_by_article_id($article_id){
		$comments = $this->fetch_one('article_comments','message','article_id ='.intval($article_id).' AND shang_cash >0', 'shang_cash desc');
		return $comments;
	}
	public function get_shang_person_num_by_article_id($article_id){
		return $this->count('shang','item_id = '.$article_id.' and item_type = "article"');
	}

	public function get_articles_list_by_topic_title($topic_title,$number = null)
	{
        $topic_id = $this->model('topic')->get_topic_by_title($topic_title)["topic_id"];
        if ($number == null)
        	return $this->fetch_all('article',' category_id = '.$topic_id,'show_index');
        else
        	return $this->fetch_all('article',' category_id = '.$topic_id ,'show_index',$number);
	}
}