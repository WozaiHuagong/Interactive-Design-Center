...
class topic_class extends AWS_MODEL
{
	...
	/* 获取·分类·-- HTML 数据 */
	public function build_topic_html($type,$parent_id = 0, $selected_id = 0, $prefix = '', $child = true)
	{

		if (!$category_list = $this->get_topic_list('parent_id = '.$parent_id.' AND type= '.'\''.$type.'\'',null,1000))
		{
			return false;
		}
		...
		{
			if ($selected_id == $val['topic_id'])
			{

				$html .= '<option value="' . $val['topic_id'] . '" selected="selected">' . $_prefix . $val['topic_title'] . '</option>';
			}
			else
			{
				$html .= '<option value="' . $val['topic_id'] . '">' . $_prefix . $val['topic_title'] . '</option>';
			}

			if ($child AND $val['parent_id'])
			{
				$html .= $this->build_topic_html($val['topic_id'], $selected_id, $prefix . '--');
			}
			else
			{
				unset($prefix);
	...
	/* 获取分类 JSON 数据 */
	public function build_topic_json($type,$parent_id = 0, $prefix = '')
	{
		if (!$category_list = $this->get_topic_list('parent_id = '.$parent_id.' AND type= '.'\''.$type.'\''))
		{
			return false;
		}
		...
		foreach ($category_list AS $category_id => $val)
		{
			$data[] = array(
				'id' => $val['topic_id'],
				'title' => $_prefix . $val['topic_title'],
				'description' => $val['topic_description'],
				'sort' => '',
	...
	public function set_type($topic_id, $type)
	{
		if (!$topic_id)
		{
			return false;
		...
		$to_update_topic['type'] = strval($type);

		if ($to_update_topic['type'] != 'article')
		{
			$to_update_topic['type'] = 'question';
		}
