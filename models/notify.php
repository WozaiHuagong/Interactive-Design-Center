...
class notify_class extends AWS_MODEL
{
    ...
    const TYPE_AD_APPROVED = 1001; // 广告通过审核
    const TYPE_AD_REMOVE   = 1002; // 广告被删除
	...
	public function list_notification($recipient_uid, $read_status = 0, $limit = null)
	{
		...
		if ($ad_ids)
		{
			$ad_list = $this->model('ad')->fetch_all('ad', 'id IN(' . implode(',', $ad_ids) . ')');
		}
		...
		foreach ($notify_list as $key => $notify)
		{
			...
			switch ($notify['model_type'])
			{
                ...
                case self::CATEGORY_AD :
                    if ($notify['action_type'] == self::TYPE_AD_APPROVED)
                    {
                        $tmp_data['title'] = $data['title'];
                    }
                    else
                    {
                        if (!$ad_list[$data['ad_id']])
                        {
                            continue;
                        }

                        $tmp_data['title'] = $ad_list[$data['ad_id']]['title'];
                    }
                    ...
                    $tmp_data['key_url'] = get_js_url('/info/' . $data['ad_id'] . '?' . implode('&', $querys));
