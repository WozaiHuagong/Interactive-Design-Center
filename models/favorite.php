...
class favorite_class extends AWS_MODEL
{
	...
	public function get_item_list($tag, $uid, $limit,$type)
	{
		if (!$uid)
		{
			return false;
		...
		{
			$favorite_items = $this->fetch_all('favorite', "type = '".$type."' AND uid = " . intval($uid), 'item_id DESC', $limit);
		}
	...
	public function process_list_data($favorite_items)
	{
		...
		foreach ($favorite_items as $key => $data)
		{
			switch ($data['type'])
			{
				...
				case 'sortout':
					$sortout_ids[] = $data['item_id'];
				break;

				case 'prize':
					$prize_ids[] = $data['item_id'];
				break;
			}
		}
		if ($answer_ids)
		...
		if ($sortout_ids)
		{
			if ($sortout_infos = $this->model('sortout')->get_sortout_info_by_ids($sortout_ids))
			{
				foreach ($sortout_infos AS $key => $data)
				{
					$favorite_uids[$data['uid']] = $data['uid'];
				}
			}
		...
		if ($prize_ids)
		{
			if ($prize_infos = $this->model('prize')->get_prize_info_by_ids($prize_ids))
			{
				//p($prize_infos);
				foreach ($prize_infos AS $key => $data)
				{
					$favorite_uids[$data['uid']] = $data['uid'];
				}
			}
		...
		foreach ($favorite_items as $key => $data)
		{
			switch ($data['type'])
			{
				...
				case 'sortout':
					$favorite_list_data[$key]['title'] = $sortout_infos[$data['item_id']]['title'];
					$favorite_list_data[$key]['link'] = get_js_url('/sortout/' . $data['item_id']);
					$favorite_list_data[$key]['add_time'] = $sortout_infos[$data['item_id']]['add_time'];

					$favorite_list_data[$key]['article_info'] = $sortout_infos[$data['item_id']];

					$favorite_list_data[$key]['last_action_str'] = ACTION_LOG::format_action_data(ACTION_LOG::ADD_ARTICLE, $data['uid'], $users_info[$data['uid']]['user_name']);

					$favorite_list_data[$key]['user_info'] = $users_info[$sortout_infos[$data['item_id']]['uid']];
				break;

				case 'prize':
					$favorite_list_data[$key]['title'] = $prize_infos[$data['item_id']]['title'];
					$favorite_list_data[$key]['link'] = get_js_url('/prize/' . $data['item_id']);
					$favorite_list_data[$key]['add_time'] = $prize_infos[$data['item_id']]['add_time'];

					$favorite_list_data[$key]['article_info'] = $prize_infos[$data['item_id']];

					$favorite_list_data[$key]['last_action_str'] = ACTION_LOG::format_action_data(ACTION_LOG::ADD_ARTICLE, $data['uid'], $users_info[$data['uid']]['user_name']);

					$favorite_list_data[$key]['user_info'] = $users_info[$prize_infos[$data['item_id']]['uid']];
				break;
			}
	...
	public function user_collected_check($uid, $item_id)
	{
		return $this->fetch_one('favorite', 'id', "uid = " . intval($uid) . " AND item_id = " . intval($item_id));
	}
	
	public function get_item_comment_list($uid, $limit,$type)
	{
		if (!$uid)
		{
			return false;
		...
		if($type == 'article'){
			$list_items = $this->fetch_all('article_comments', "uid = " . intval($uid), 'add_time DESC', $limit);
		}

		if($type == 'sortout'){
			$list_items = $this->fetch_all('sortout_comments', "uid = " . intval($uid), 'add_time DESC', $limit);
		}
