...
class system_class extends AWS_MODEL
{
	...
	public function fetch_home_data($type, $parent_id = 0, $order = 'sort ASC,id ASC')
	{
		static $category_list_all;
		...
		if (!$category_list_all[$type])
		{
			if ($type)
			{
				$category_list_all_query = $this->fetch_all('home', '`type` = \'' . $this->quote($type) . '\'', $order);
			}
			else
			{
				$category_list_all_query = $this->fetch_all('home', '', $order);
			}
		...
		return $category_list_all_query;
	}
	...
	/* 获取分类 HTML 数据 */
	public function build_home_html($type, $parent_id = 0, $selected_id = 0, $prefix = '', $child = true)
	{
		if (!$category_list = $this->fetch_home($type, $parent_id))
		{
			return false;
		}
		foreach ($category_list AS $category_id => $val)
		{
			...
			if ($selected_id == $val['parent_id'])
			{	
				$name = get_home_type_name($val['parent_id']);
				$html .= '<option value="' . $val['parent_id'] . '" selected="selected">' . $name . '</option>';
			}
			else
			{
				$name = get_home_type_name($val['parent_id']);
				$html .= '<option value="' . $val['parent_id'] . '">' .$name . '</option>';
			}
		}
		...
		/* 获取分类数组 */
	public function fetch_home($type, $parent_id = 0)
	{
		$category_list = array();

		if (!$category_all = $this->fetch_home_data($type, $parent_id))
		{
			return $category_list;
		}
		...
		foreach ($category_all AS $key => $val)
		{
			...
			{
				$val['icon'] = get_setting('upload_url') . '/home/' . $val['icon'];
			}
			...
			//if ($child_list = $this->fetch_home($type, $val['id']))
			//{
				//$category_list[$val['id']]['child'] = $child_list;
			//}
		}
	...
	/* 获取首页 JSON 数据 */
	public function build_home_json($type, $parent_id = 0, $prefix = '')
	{
		...
		foreach ($category_list AS $category_id => $val)
		{
			...
			// if ($val['child'])
			// {
			// 	$data = array_merge($data, json_decode($this->build_home_json($type, $val['id'], $prefix . '--'), true));
			// }
			// else
			// {
			// 	unset($prefix);
	...
	/* 获取首页数组信息 */
	public function get_home_info($category_id)
	{
		static $all_category;

		if (!$all_category)
		{
			if ($all_category_query = $this->fetch_all('home'))
			{
				foreach ($all_category_query AS $key => $val)
				{
					if (!$val['url_token'])
