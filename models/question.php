...
class question_class extends AWS_MODEL
{
	...
	*负数的category为特殊的问题
	*/
	public function get_special_question_info($category_id)
	{
		...
		$questions = $this->fetch_row('question', 'category_id = ' . intval($category_id));
	...
	public function update_question($question_id, $question_content, $question_detail, $uid, $verified = true, $modify_reason = null, $anonymous = null, $category_id = null,$thankmoney,$free_question = null)
	{
		...
		$this->delete('topic_relation', 'item_id = ' . intval($question_id) . " AND `type` = 'question'");
        $this->model('topic')->save_topic_relation($uid,$category_id,$question_id,'question');
	 ...
	 * @return int
	 */
	public function get_shang_person_num_by_question_id($question_id)
	{
		$sql='SELECT COUNT(*) as n FROM (SELECT COUNT(uid) as n FROM '.$this->get_table('question_shang').' WHERE question_id = '.intval($question_id).' GROUP BY uid) s';
	 ...
	 * @return array
	 */
	public function get_question_shang($limit=5,$offset=null,$question_id=null)
	{
		$sql='SELECT qs.question_id as qid,sum(cash) as cash2,max(qs.add_time) as order_time  FROM '.$this->get_table('question_shang').' AS qs Right JOIN '.$this->get_table('question').' AS q ON qs.question_id=q.question_id';
		if ($question_shang = $this->query_all($sql,$limit,$offset,'(qs.type = "question" OR qs.type="answer") AND qs.cash !=0'.($question_id?' AND qs.question_id = '.$question_id:''),'qid','order_time desc'))
		{
			foreach ($question_shang as $key => $value) {
				$question_info = $this->model('question')->get_question_info_by_id($value['qid']);
				$question_shang[$key]['question_info']=$question_info;
			}
		}
		return $question_shang;
	}
	/**
	 *
	 * 得到用户感兴趣问题列表
	 ...
	 * 
	 */
	function get_question_list($limit = null)
	{
		
		$sql = "SELECT * FROM " . get_table('question') . " WHERE 1=1 ORDER BY question_id DESC";
	...
	public function get_shang_answer_list_by_article_ids($question_id){
		$answers = $this->fetch_all('answer','question_id ='.intval($question_id).' AND shang_cash >0', 'shang_cash desc');
		return $answers;
	}
}
