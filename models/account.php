...
class account_class extends AWS_MODEL
{
     ...
     * @param  string
     */
    public function update_user_pay_password($password, $uid, $oldpassword = null)
    {
        if (!$uid)
        {
            return false;
        ...
        if(!$user = $this->fetch_row('users', 'uid = ' . intval($uid))){
            return false;
        }

        if($oldpassword){
            $oldpassword = compile_password($oldpassword, $user['salt']);
            if($user['pay_password']){
                if($oldpassword != $user['pay_password']){
                    return false;
                }
            }else{
        ...
        $this->update('users', array(
            'pay_password' => compile_password($password, $user['salt']),
        ), 'uid = ' . intval($uid));

        return true;
    }

    public function check_pay_password($password, $uid){
        if (!$uid OR !$password){
            return false;
        }
        ...
        $oldpassword = compile_password($password, $user['salt']);
        if($user['pay_password']){
            if($oldpassword == $user['pay_password']){
                return true;
            }
