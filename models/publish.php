...
class publish_class extends AWS_MODEL
{
	...
	public function publish_ab_answer($question_id, $answer_content, $uid, $anonymous = null, $attach_access_key = null, $auto_focus = true, $reply_to_openid = true)
	{
		...
		if (!$answer_id = $this->model('abc')->save_answer($question_id, $answer_content, $uid, $anonymous))
		{
			return false;
		}
		...
		$this->model('abc')->save_last_answer($question_id, $answer_id);
	...
	public function publish_ab_question($question_content, $question_detail, $category_id, $uid, $topics = null, $anonymous = null, $attach_access_key = null, $ask_user_id = null, $create_topic = true, $toShow = null)
	{
		if ($question_id = $this->model('abc')->save_question($question_content, $question_detail, $uid, $anonymous, null, $toShow))
		{
			...
			if ($attach_access_key)
			{
					$this->model('publish')->update_attach('ab_question', $question_id, $attach_access_key);
			}
	...
	public function publish_article($title, $message, $uid, $topics = null, $category_id = null, $attach_access_key = null, $create_topic = true,$award_price, $is_original, $is_award, $summary,$other_category,$show_index)
	{
	...
	public function publish_sortout($title, $message, $uid, $topics = null, $category_id = null, $attach_access_key = null, $create_topic = true,$award_price, $is_original, $is_award, $summary,$sourcecontent,$question_id,$other_category)
	{
		if($category_id == 0 && !empty($other_category)){
			 $category_id = $this->model('category')->add_category('question', $other_category, 0);
		}
		if ($article_id = $this->insert('sortout', array(
			'uid' => intval($uid),
			'title' => htmlspecialchars($title),
			'message' => htmlspecialchars($message),
			'category_id' => intval($category_id),
			...
			'is_original'  => intval($is_original),
			'summary' => htmlspecialchars($summary),
			'source_content' => htmlspecialchars($sourcecontent),
			'question_id' => intval($question_id),
			'add_time' => time()
		)))
		{
			...
			if (is_array($topics))
			{
				foreach ($topics as $key => $topic_title)
				{
					...
					$this->model('topic')->save_topic_relation($uid, $topic_id, $article_id, 'sortout');
				}
			}

			if ($attach_access_key)
			{
				$this->model('publish')->update_attach('sortout', $article_id, $attach_access_key);
			}

			$this->model('search_fulltext')->push_index('sortout', $title, $article_id);

			// 记录日志
			ACTION_LOG::save_action($uid, $article_id, ACTION_LOG::CATEGORY_QUESTION, ACTION_LOG::ADD_ARTICLE, $title, $message, 0);

			$this->model('posts')->set_posts_index($article_id, 'sortout');
	...
	//有奖
	public function publish_prize($title, $message, $uid, $topics = null, $category_id = null, $attach_access_key = null, $create_topic = true,$award_price, $is_original, $is_award, $summary,$other_category)
	{
		if($category_id == 0 && !empty($other_category)){
			 $category_id = $this->model('category')->add_category('question', $other_category, 0);
		}
		if ($article_id = $this->insert('prize', array(
			'uid' => intval($uid),
			'title' => htmlspecialchars($title),
			'message' => htmlspecialchars($message),
			'category_id' => intval($category_id),
		...
		)))
		{
			...
			if ($attach_access_key)
			{
				$this->model('publish')->update_attach('prize', $article_id, $attach_access_key);
			}

			$this->model('search_fulltext')->push_index('prize', $title, $article_id);
			...
			$this->shutdown_update('users', array(
				'article_count' => $this->count('prize', 'uid = ' . intval($uid))
			), 'uid = ' . intval($uid));
		}
	...
	public function publish_sortout_comment($article_id, $message, $uid, $at_uid = null)
	{
		if (!$article_info = $this->model('sortout')->get_sortout_info_by_id($article_id))
		{
			return false;
		}

		$comment_id = $this->insert('sortout_comments', array(
			'uid' => intval($uid),
			'article_id' => intval($article_id),
			'message' => htmlspecialchars($message),
			'add_time' => time(),
		...
		$this->update('sortout', array(
			'comments' => $this->count('sortout_comments', 'article_id = ' . intval($article_id))
		), 'id = ' . intval($article_id));
		...
		if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_uid($article_info['uid']) AND $article_info['uid'] != $uid)
		{
			...
			if ($weixin_user_info['weixin_settings']['NEW_ARTICLE_COMMENT'] != 'N')
			{
				$this->model('weixin')->send_text_message($weixin_user['openid'], "您的文章 [" . $article_info['title'] . "] 收到了新的评论:\n\n" . strip_tags($message), $this->model('openid_weixin_weixin')->redirect_url('/sortout/' . $article_info['id']));
			}
		}

		ACTION_LOG::save_action($uid, $article_info['id'], ACTION_LOG::CATEGORY_QUESTION, ACTION_LOG::ADD_COMMENT_ARTICLE, $message, $comment_id);

		$this->model('posts')->set_posts_index($article_info['id'], 'sortout');

		return $comment_id;
	}
	
	public function publish_prize_comment($article_id, $message, $uid, $at_uid = null)
	{
		...
		$comment_id = $this->insert('prize_comments', array(
			'uid' => intval($uid),
			'article_id' => intval($article_id),
			'message' => htmlspecialchars($message),
			'add_time' => time(),
		...
		$this->update('prize', array(
			'comments' => $this->count('prize_comments', 'article_id = ' . intval($article_id))
		), 'id = ' . intval($article_id));
		...
		if ($weixin_user = $this->model('openid_weixin_weixin')->get_user_info_by_uid($article_info['uid']) AND $article_info['uid'] != $uid)
		{
			...
			if ($weixin_user_info['weixin_settings']['NEW_ARTICLE_COMMENT'] != 'N')
			{
				$this->model('weixin')->send_text_message($weixin_user['openid'], "您的文章 [" . $article_info['title'] . "] 收到了新的评论:\n\n" . strip_tags($message), $this->model('openid_weixin_weixin')->redirect_url('/prize/' . $article_info['id']));
			}
		}

		ACTION_LOG::save_action($uid, $article_info['id'], ACTION_LOG::CATEGORY_QUESTION, ACTION_LOG::ADD_COMMENT_ARTICLE, $message, $comment_id);

		$this->model('posts')->set_posts_index($article_info['id'], 'prize');
