...
FileUpload.prototype = 
{
	...
	addFileList : function (input)
	{
		...
		if (files && (this.type == 'file' || this.type == 'attach'))
		{
		...
		if (json)
		{
			if (!json.error)
			{
				...
				if (this.options.insertBtn && json.delete_url && !json.class_name)
				{
					...
					var btn = this.createInsertBtn(json.thumb);
					//console.log(json);
					$(element).find('.meta').append(btn);
				}
    ...
    createInsertBtn : function (attach_id)
    {
    	...
    	$(btn).click(function()
		{
			if (typeof CKEDITOR != 'undefined')
			{
				...
				//return false;
				//!!!!!!
				_this.editor.imageElement = _this.editor.document.createElement( 'img' );
                _this.editor.imageElement.setAttribute( 'src', str2 );
                // console.log(_this.editor.imageElement.$.outerHTML)
                // _this.editor.insertElement( _this.editor.imageElement );//原来的
