...
$(document).ready(function ()
{
    ...
    //文章页添加评论, 话题添加 绑定事件
    AWS.Init.init_article_comment_box('#article_box .aw-article-content .aw-article-comment');
    AWS.Init.init_sortout_comment_box('#sortout_box .aw-article-content .aw-article-comment');
    AWS.Init.init_prize_comment_box('#prize_box .aw-article-content .aw-article-comment');
    ...
    AWS.Dropdown.bind_dropdown_list('#aw-search-query-fxd', 'search');
    ...
    //广告点击初始化
    if($('[data-toggle="ad"]').length){
        $('[data-toggle="ad"]').each(function (i, item) {
            var ad_url = $(item).data('id');
            if(ad_url){
                $(item).attr('href', ad_url);
            }
        });
    }
});
