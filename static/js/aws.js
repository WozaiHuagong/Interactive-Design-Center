var AWS =
{
	...
	ajax_processer: function (type, result)
	{
		...
		else
		{
			...
			if (type == 'ab_comments_form')
			{
				AWS.reload_ab_comments_list(result.rsm.item_id, result.rsm.item_id, result.rsm.type_name);
				$('#aw-comment-box-' + result.rsm.type_name + '-' + result.rsm.item_id + ' form textarea').val('');
				$('.aw-comment-box-btn .btn-success').removeClass('disabled');
			}
	...
	// 重新加载评论列表
	reload_ab_comments_list: function(item_id, element_id, type_name)
	{
		$('#aw-comment-box-' + type_name + '-' + element_id + ' .aw-comment-list').html('<p align="center" class="aw-padding10"><i class="aw-loading"></i></p>');

		$.get(G_BASE_URL + '/abc/ajax/get_' + type_name + '_comments/' + type_name + '_id-' + item_id, function (data)
		{
			$('#aw-comment-box-' + type_name + '-' + element_id + ' .aw-comment-list').html(data);
		});
	},
	...
	dialog: function (type, data, callback)
	{
		switch (type)
		{
			...
			case 'commentEdit':
				...
				break;
			case 'ABcommentEdit':
				var template = Hogan.compile(AW_TEMPLATE.editABCommentBox).render(
				{
					'answer_id': data.answer_id,
					'attach_access_key': data.attach_access_key
				});			
			...
			//收藏
			case 'collected':
				var template = Hogan.compile(AW_TEMPLATE.collectBox).render(
				{
					 'item_id': data.item_id,
					 'item_type': data.item_type
				});
			...
			case 'CBox':
				var template = Hogan.compile(AW_TEMPLATE.CBox).render(
				{
					'item_type': 'C',
					'item_id': data.item_id,
					'item_value': data.item_value
		...
		if (template)
		{
			...
			switch (type)
			{
				...
				case 'publish':
					...
					if ($('#aw-search-query-fxd').val() && $('#aw-search-query-fxd').val() != $('#aw-search-query-fxd').attr('placeholder'))
					{
						$('#quick_publish_question_content').val($('#aw-search-query-fxd').val());
					}
				...
				case 'publish_append_C':
					...
					// $('#quick_publish')[0].action='/append/ajax/public_C';
					...
					$('#myModalLabel').innerHTML='发送消息到后台';
					$('div>a[onclick]').hide();
					$('.aw-publish-box .aw-publish-title').hide();
					$('.aw-edit-topic-box').hide();
					$('#quick_publish .close-edit').hide();
				...
				case 'collected':
					if ($('.collected').hasClass('current'))
					{
						$.post(G_BASE_URL + '/favorite/ajax/remove_favorite_item/',
						{
							'item_id' : $('.collected input[name="item_id"]').val(),
							'item_type' : $('.collected input[name="item_type"]').val(),
							'tags' : ''
						}, function (result)
						{
							if (result.errno == 1)
							{
								$('.collected').find('span').html(_t('收藏'));
								$('.collected').removeClass('current');
							}
						}, 'json');
					}
					else
					{
						...
						}, function (result)
						{
							if (result.errno == 1)
							{
								$('.collected').find('span').html(_t('已收藏'));
								$('.collected').addClass('current');
							}
						}, 'json');
					}
				break;
				...
				case 'commentEdit':
					...
					break;
				case 'ABcommentEdit':
					$.get(G_BASE_URL + '/abc/ajax/fetch_answer_data/' + data.answer_id, function (result)
					{
						console.log(result)
						$('#editor_reply').html(result.answer_content.replace('&amp;', '&'));
						...
						{
							var fileupload = new FileUpload('file', '.aw-edit-comment-box .aw-upload-box .btn', '.aw-edit-comment-box .aw-upload-box .upload-container', G_BASE_URL + '/publish/ajax/attach_upload/id-answer__attach_access_key-' + ATTACH_ACCESS_KEY, {'insertTextarea': '.aw-edit-comment-box #editor_reply', 'editor' : editor});
							$.post(G_BASE_URL + '/publish/ajax/ab_answer_attach_edit_list_action/', 'answer_id=' + data.answer_id, function (data) {
								if (data['err']) {
									return false;
								} else {
									$.each(data['rsm']['attachs'], function (i, v) {
...
AWS.User =
{
	...
	agree_vote: function(selector, user_name, answer_id)
	{
		...
		else
		{
			...
			{
				$(selector).parents('.aw-item').find('.aw-agree-by').append('<a style="display:none;" class="aw-user-name">' + user_name + '</a>');
			}
			else
			{
				$(selector).parents('.aw-item').find('.aw-agree-by').append('<em>、</em><a style="display:none;" class="aw-user-name">' + user_name + '</a>');
			}
	...
	// 提交评论
	save_ab_comment: function(selector)
	{
		selector.addClass('disabled');

		AWS.ajax_post(selector.parents('form'), AWS.ajax_processer, 'ab_comments_form');
	},
	...
	prize_comment_vote: function(selector, comment_id, rating)
	{
		AWS.loading('show');
		...
		$.post(G_BASE_URL + '/prize/ajax/prize_vote/', 'type=comment&item_id=' + comment_id + '&rating=' + rating, function (result)
		{
			AWS.loading('hide');
	...
	sortout_comment_vote: function(selector, comment_id, rating)
	{
		AWS.loading('show');
		...
		$.post(G_BASE_URL + '/sortout/ajax/sortout_vote/', 'type=comment&item_id=' + comment_id + '&rating=' + rating, function (result)
		{
			AWS.loading('hide');
	...
	// ad赞同
	info_vote: function(selector, ad_id, rating)
	{
		AWS.loading('show');
		...
		$.post(G_BASE_URL + '/info/ajax/info_vote/', 'type=info&item_id=' + ad_id + '&rating=' + rating, function (result) {
...
AWS.Init =
{
	...
	// 初始化问题评论框
	init_ab_comment_box: function(selector)
	{
		$(document).on('click', selector, function ()
		{
			$(this).parents('.aw-question-detail').find('.aw-invite-box, .aw-question-related-box').hide();
			...
			else
			{
				...
				{
					case 'question':
						var comment_form_action = G_BASE_URL + '/abc/ajax/save_question_comment/question_id-' + $(this).attr('data-id');
						var comment_data_url = G_BASE_URL + '/abc/ajax/get_question_comments/question_id-' + $(this).attr('data-id');
						break;

					case 'answer':
						var comment_form_action = G_BASE_URL + '/abc/ajax/save_answer_comment/answer_id-' + $(this).attr('data-id');
						var comment_data_url = G_BASE_URL + '/abc/ajax/get_answer_comments/answer_id-' + $(this).attr('data-id');
						break;
				}

				if (G_USER_ID)
				{
					$(this).parents('.aw-item').find('.mod-footer').append(Hogan.compile(AW_TEMPLATE.ABcommentBox).render(
					{
						'comment_form_id': comment_box_id.replace('#', ''),
						'comment_form_action': comment_form_action
					}));
	...
	//初始化整理
	init_sortout_comment_box: function(selector)
	{
		$(document).on('click', selector, function ()
		{ 
			var _editor_box = $(this).parents('.aw-item').find('.aw-article-replay-box');
			...
			{
				$(this).parents('.mod-footer').append(Hogan.compile(AW_TEMPLATE.sortoutCommentBox).render(
				{
					'at_uid' : $(this).attr('data-id'),
					'article_id' : $('.aw-topic-bar').attr('data-id')
				}));
	...
	init_prize_comment_box: function(selector)
	{
		$(document).on('click', selector, function ()
		{ 
			var _editor_box = $(this).parents('.aw-item').find('.aw-article-replay-box');
			...
			{
				$(this).parents('.mod-footer').append(Hogan.compile(AW_TEMPLATE.prizeCommentBox).render(
				{
					'at_uid' : $(this).attr('data-id'),
					'article_id' : $('.aw-topic-bar').attr('data-id')
				}));
