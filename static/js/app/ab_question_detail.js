...
{
    //问题页添加评论
    AWS.Init.init_ab_comment_box('.aw-add-comment');

	if ($('#c_log_list').attr('id'))
	{
		AWS.load_list_view(G_BASE_URL + '/abc/ajax/log/id-' + QUESTION_ID, $('#bp_log_more'), $('#c_log_list'));
	}
	else
	{
	...
	//关注用户列表
	$.get(G_BASE_URL + '/abc/ajax/get_focus_users/question_id-' + QUESTION_ID, function (result) {
		if (result)
		{
